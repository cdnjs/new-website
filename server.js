...
MongoClient.connect(process.env.MONGOHQ_URL, function(err, db) {
  ...
  */
  ...
  app.get('/libraries/:library', function(req, res) {
    setCache(res, 1);
    db.collection('updates').find({"status" : {$regex : ".*http.*"}}, {'limit':3, 'sort':[['sponsored', -1], ['posted_at', -1]]}).toArray(function(err, docs) {
  ...
  app.get('/news', function(req, res) {
    db.collection('updates').find({}, {"sort": [["posted_at", -1]]).toArray(function(err, docs) {
      console.log('Number of Updates --------------------------------------- ', docs.length);
      _.each(docs, function(doc){
        doc.posted_at = new Date(doc.posted_at);
        doc.status = linkify(doc.status);
